...
The official plugin is the recommended path. The current marketplace plugin is version 0.5.6 and bundles Core 0.6.4:
...
```text
uvx --from memtomem[onnx]==0.6.4 memtomem-server
```
...
- **Keep the plugin commands with the manual server:** keep the plugin installed and register the manual entry with the exact plugin signature. For example:
  ...
  ```bash
  claude mcp add memtomem -- uvx --from "memtomem[onnx]==0.6.4" memtomem-server
  ```
