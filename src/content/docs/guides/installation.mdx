...
`[all]` is the recommended first install because it matches the main docs: ONNX dense embeddings, Korean tokenizer, Ollama / OpenAI providers, code chunker, the Web UI, and Langfuse observability tracing (~250 MB). For a leaner footprint, drop the extras for a BM25-only install (~40 MB):
