...
STM's proxied tools surface as `<prefix>__<tool>`, and your client composes them into a longer `mcp__<server>__<prefix>__<tool>` name. That name has a 64-char limit, so if a tool goes missing, register STM under a short client name like `mms` (as above) **and** export `MMS_CLIENT_SERVER_NAME=mms` so STM's overflow check agrees — `mms init --mcp claude` registers under the longer name `memtomem-stm` by default. See [Troubleshooting](/guides/troubleshooting/) if tools disappear.
