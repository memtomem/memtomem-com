...
<Tabs syncKey="mcp-client">
  <TabItem label="Claude Code">
    If you picked "auto-register with Claude Code" in the `mm init` wizard, LTM (`memtomem-server`) is already registered. STM (`memtomem-stm`) was also registered by `mms init --mcp claude` in Step 2 — no extra command needed.
