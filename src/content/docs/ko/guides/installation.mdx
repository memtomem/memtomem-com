...
`[all]`은 이 문서의 기본 흐름과 맞는 권장 설치입니다. ONNX 로컬 임베딩, 한국어 토크나이저, Ollama / OpenAI 프로바이더, 코드 청커, Web UI, Langfuse 관측성 트레이싱을 모두 포함합니다(~250 MB). 가볍게 설치하려면 extras 를 생략해 BM25 전용(~40 MB)으로 설치할 수 있습니다:
...
STM은 선택 사항입니다. 능동적 기억 제시(surfacing)나 도구 응답 압축이 필요할 때, LTM 빠른 시작 흐름이 동작한 뒤 설치하세요.
