...
대화형 설정 마법사를 실행합니다. 시작 시 **프리셋 피커**(Minimal / English / Korean)가 임베딩·리랭커·토크나이저·네임스페이스 기본값을 한 번에 적용하고, 이어서 데이터베이스 경로 등을 질의합니다. `-y`로 모든 기본값 수락(= `--preset minimal -y`), `--preset korean` / `--preset english`로 비대화 선택도 가능합니다.

```bash
mm init                            # 대화형 설정 + 프리셋 피커
mm init -y                         # 기본값 자동 수락 (CI 용) — Minimal 프리셋
mm init --preset korean            # 한국어 프리셋 선택 후 나머지 단계 대화형
```
