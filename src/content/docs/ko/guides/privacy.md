...
memtomem은 기본적으로 기억 데이터를 외부로 보내지 않습니다. 저장·임베딩·검색·재정렬이 모두 내 컴퓨터에서 일어나며, 비밀값으로 보이는 내용은 캐시·색인·공유 단계에서 자동으로 걸러집니다.
...
- **유일한 예외** — 외부 제공자를 선택했을 때만 데이터가 기기를 벗어납니다: OpenAI·Ollama 임베딩/재정렬, 또는 STM의 외부 LLM 압축·추출 경로. STM 외부 LLM 경로에서는 기본값 `privacy_scan_enabled`(켜짐)가 외부 호출 전에 자격증명을 스캔해 감지 시 호출을 건너뛰며, 이를 끄면 원시 응답이 스캔 없이 전송됩니다(서버가 시작 시 경고를 남깁니다).
