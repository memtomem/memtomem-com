...
- **유일한 예외** — 외부 제공자를 선택했을 때만 데이터가 기기를 벗어납니다. 두 가지 경우가 있습니다: OpenAI·Ollama 임베딩/재정렬을 쓸 때, 또는 STM이 외부 LLM으로 압축·추출을 수행할 때입니다. STM의 외부 LLM 경로에서는 기본값인 `privacy_scan_enabled`(켜짐)가 외부 호출 직전에 자격증명이 있는지 검사해, 발견되면 그 호출을 건너뜁니다. 이 설정을 끄면 응답 원문이 검사 없이 그대로 전송됩니다(이 경우 서버가 시작할 때 경고를 남깁니다).
