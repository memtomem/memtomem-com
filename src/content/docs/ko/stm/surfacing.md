...
서피싱된 기억을 응답에 붙이는 방식은 `MEMTOMEM_STM_SURFACING__INJECTION_MODE`로 제어합니다.

| 모드 | 동작 |
|---|---|
| `prepend` (기본) | 기억을 응답 헤더로 앞에 붙입니다. Progressive continuation에서는 Stage 3 서피싱을 건너뜁니다. |
| `append` | 기억을 응답 아래에 붙입니다. Progressive continuation에서도 서피싱을 실행합니다. |
| `section` | 기억을 별도 섹션에 배치합니다. Progressive continuation에서도 서피싱을 실행합니다. |
...
- **회로 차단기** (3-state) — 연속 실패 시 서피싱 일시 중단, 기본 60초 후 half-open 재시도
- **서피싱 타임아웃** — 호출당 기본 `3s` 상한
- **Rate limit** — 전체 도구 기준 기본 `15 calls / minute`
- **쓰기 도구 스킵** — 파일 쓰기, 삭제 등 부수효과가 있는 도구에서는 서피싱 비활성화
- **쿼리 쿨다운** — 최근 5초 안에 본 쿼리와 Jaccard 유사도 `> 0.95`이면 스킵
- **교차 세션 중복 제거** — 기본 TTL `604800s`(7일), `MEMTOMEM_STM_SURFACING__DEDUP_TTL_SECONDS`로 조정
- **주입 크기 상한** — 기본 `3000 chars`
