...
- **노출 도구 정리** — STM은 연결한 서버의 도구를 모두 그대로 넘기지 않고, 에이전트에 보여 줄 목록을 정리합니다. 응답하지 않는 서버의 도구, 설명에 자격 증명으로 보이는 문자열이 있는 도구, 이름이 겹치거나 길이 제한을 넘는 도구는 제외합니다. 노출 정책은 `exposure.profile`(`strict` 기본 / `review` / `explore`)로 조정합니다. `stm_admin(action="proxy_health")`에서는 발견한 도구 수와 실제로 보여 준 도구 수(`discovered N / advertised M`)를 확인할 수 있습니다.
