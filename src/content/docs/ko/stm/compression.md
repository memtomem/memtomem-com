...
모든 MCP 도구 응답은 에이전트에 전달되기 전에 STM을 거칩니다. 응답이 에이전트의 컨텍스트 예산을 초과할 경우 STM이 압축을 수행하며, 데이터의 형태에 따라 압축 방식이 달라집니다. JSON과 마크다운은 동일한 방식으로 압축할 수 없으며, 500KB 규모의 API 스키마와 20단락 분량의 설계 문서는 서로 다른 처리 대상입니다.
