---
title: 압축 전략
description: 8종 압축 전략의 동작 원리, 자동 선택 로직, 쿼리 인식 예산 배분.
---
...
memtomem-stm은 MCP 도구 응답을 콘텐츠 유형에 따라 자동으로 압축하여 토큰을 절감합니다. 에이전트가 필요로 하는 정보를 유지하면서 응답 크기를 축소하는 8가지 전략과, 응답별로 적절한 전략을 고르는 자동 선택(`auto`)을 제공합니다. 전략 선택이 어려운 경우 `auto` 설정을 유지하면 즉시 응답형 전략 중에서 응답별로 적절한 전략이 자동 선택됩니다.
...
`llm_summary` 전략은 별도의 **타임아웃 가드**를 가집니다: 서버·도구별 `llm` 블록의 `llm_timeout_seconds` 필드(기본 `60`초). 느리거나 멈춘 LLM 엔드포인트가 더 이상 프록시 전체를 멈추지 않으며 — 타임아웃 발생 시 STM이 `truncate` 로 폴백해 에이전트에 한정된 길이의 응답이 반환됩니다.
