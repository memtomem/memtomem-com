...
`progressive` 전략은 보관된 소스를 여러 번에 나눠 전달하며, 아래 복구 범위와 만료 조건이 적용됩니다.
...
점진 전달은 업스트림 제한 적용 후 정리된 소스를 보관합니다. 원본 응답 전체를 무제한·바이트 단위로 보존하는 기능은 아닙니다. 대기 중 선택 항목은 기본적으로 메모리에 저장되며 SQLite 영속 저장은 옵트인입니다. TTL이 지나 만료된 ID는 오류를 반환하고 업스트림을 자동 재호출하지 않습니다. 원래의 읽기 전용 호출을 다시 수행하는 것은 별도 명시적 동작입니다. 콘텐츠 복구를 위해 쓰기 작업을 반복하지 마세요.
