...
설정 마법사는 먼저 **프리셋 선택 화면**(Minimal / English / Korean)을 보여 줍니다. 프리셋은 임베딩·리랭커·토크나이저·네임스페이스 기본값을 한 번에 적용합니다. 입력 없이 실행하려면 `--preset <name>`을 지정하세요. `--advanced`는 프리셋 선택을 건너뛰고 10단계 전체 설정을 엽니다.
