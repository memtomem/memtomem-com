...
리랭커가 활성화되면 후보 풀 크기는 `max(min_pool, min(max_pool, int(oversample * response_top_k)))`로 계산됩니다. 기본값(oversample `2.0`, min_pool `20`, max_pool `200`)은 `top_k=10`에서 고전적인 2× 오버샘플링(풀 20)을 제공하면서 더 큰 `top_k` 요청에서는 자연스럽게 확장됩니다.
