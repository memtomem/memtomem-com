...
현재 holdout은 이식 가능한 qrel과 고정된 corpus/query hash를 사용해
영문·한글·교차 언어 트랙의 이중 언어 쿼리 120개를 평가합니다. 기존
48개 파일·192개 청크·100개 쿼리 회귀 포트폴리오를 대체하지 않고
보완합니다.

단일 실행 staged k-sweep에서는 제품 기본값 `top_k=10`, BM25/dense 후보
`50/50`, `rrf_k=60`, reranker 비활성 상태를 유지했습니다. `top_k=5`의
후보 폭 100은 후속 검증 후보일 뿐이며, 기본값 변경 전에 5회/10회 반복
검증이 필요합니다.
